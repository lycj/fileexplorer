...
namespace QuickZip.Converters
{
    ...
    public abstract partial class IconConverterBase : IValueConverter, IMultiValueConverter
    {
        ...
        internal static SystemImageListCollection sysImgList = new SystemImageListCollection();

        public static Bitmap GetFileIcon(IntPtr fileName, bool isDirectory, bool forceLoad, IconSize size)
        {
            switch (size)
            {
                ...
                case IconSize.jumbo:
                    if (sysImgList[size] != null)
                    {
                        Bitmap retImage = sysImgList[size][fileName, isDirectory, forceLoad];
                        if (checkImage(retImage))
                            return resizeImage(retImage, IconSizeToSize(size), 0);
                    }
                    break;
            }
        ...
        private static bool checkImage(Bitmap bitmap)
        {
        ...
        /// </summary>
        public static Bitmap GetFileIcon(string fileName, bool isDirectory, bool forceLoad, IconSize size)
        {
        ...
        protected static Dictionary<string, BitmapSource> iconDic = new Dictionary<string, BitmapSource>();
        ...
        {
            while (iconDic.Count > 50)
                iconDic.Remove(iconDic.Keys.First());
            //iconDic.Clear();
            currentRoll += 1;
        }
