...
//      		cm.Matrix40=cm.Matrix41=cm.Matrix42=.04f;
        ...
        /// </summary>
        public static bool Run2(string prog, string param, ref QuickZip.MiniHtml.ProcessInfo pi)
        {
            //Int32 Infinite = Int32.MaxValue;
            //Int32 WAIT_OBJECT_0 = 0;
            
            Byte[] si = new Byte[128];
            if (pi == null) { pi = new QuickZip.MiniHtml.ProcessInfo(); }
        ...
        {        	
        	return (((chs.tagName == CssHeaderStyleType.UnspecifiedTagName) ||
        	          ...
        	          ((chs.tagClass == "") || (chs.tagClass == input.variables["class"].value)) &&
        	          ((chs.tagID == "") || (chs.tagID == input.variables["id"].value)));        	        	
        }
        private static bool haveOtherClassID(HtmlTag input, CssHeaderStyleType chs)
        {
        	bool retVal = false;
        	if ((chs.tagClass != "") && (input.variables["class"].value != ""))
        	    retVal = (chs.tagClass == input.variables["class"].value);
        	if ((chs.tagID != "") && (input.variables["id"].value != ""))
        	    retVal = (chs.tagID == input.variables["id"].value);
        	return retVal;
        }
        /// <summary>
        ...
        internal static bool MatchCurrentTag(HtmlTag currentTag, string header)
        {
        	...
        	retVal = retVal && ((chs.tagClass == "") || 
        	                    ((currentTag.variables.Contains("class")) && 
        	                     (chs.tagClass == currentTag.variables["class"].value)));
        	retVal = retVal && ((chs.tagID == "") || 
        	                    ((currentTag.variables.Contains("id")) &&
        	                                         (chs.tagID == currentTag.variables["id"].value)));
